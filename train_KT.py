...
def main(args):
    ...
    if cfg.network == "iresnet100":
        ...
        exit()
    try:
        backbone_pth = os.path.join(cfg.output_real)
    ...
    for ps in backbone.parameters():
        dist.broadcast(ps, 0)
    #for ps in backbone_t.parameters():
    #    dist.broadcast(ps, 0)
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for _, (img, label) in enumerate(train_loader):
            ...
            with torch.no_grad():
                features_t=F.normalize(backbone_t(img))

            alpha=0.1
            #loss_v = criterion(thetas, label) + 10000* criterion_mse(features,features_t)
            loss_v =  criterion_mse(features,features_t)
            loss_v.backward()
